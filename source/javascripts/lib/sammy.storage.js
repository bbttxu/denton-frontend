(function(factory) {
  ...
  $.extend(Sammy.Store.SessionStorage.prototype, {
    isAvailable: function() {
      return ('sessionStorage' in window) && (window.location.protocol != 'file:') && ($.isFunction(window.sessionStorage.setItem));
    },
    exists: function(key) {
      return (this.get(key) != null);
    },
  ...
  $.extend(Sammy.Store.Cookie.prototype, {
    ...
    _setCookie: function(key, value, expires) {
      ...
      var set_cookie = [
      this._key(key), "=", value, "; expires=", date.toGMTString(), "; path=", this.path].join('');
  ...
  Sammy.Session = function(app, options) {
    ...
    this.store('session', $.extend({
      type: ['local', 'cookie', 'memory']
    }, options));
  };
