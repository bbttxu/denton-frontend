  ...
  var Route = function(method, path, handlers) {
      var convertPathToRegExp = function() {
          if (!(path instanceof RegExp)) {
            var str = path.replace(pathNameRegex, pathNameReplacement).replace(splatNameRegex, splatNameReplacement);
...
Davis.App = (function() {
  ...
  App.prototype.start = function() {
    ...
    var beforeFiltersPass = function(request) {
        ...
        self.lookupBeforeFilter(request.method, request.path), runFilterWith(request))
      }
    ...
    var handleRequest = function(request) {
        if (beforeFiltersPass(request)) {
          ...
          if (route) {
            ...
            Davis.utils.every(
            self.lookupAfterFilter(request.method, request.path), runFilterWith(request));
    ...
    var bindToInternalEvents = function() {
        self.bind('runRoute', function(request) {
